...
from utils.utils import MAP_MODELS_COORDS, generate
...
def configure_subparsers(subparsers: argparse._SubParsersAction):
    ...
    parser = subparsers.add_parser(
        "ablations",
        help="Perform ablation studies on PtC models.",
    ...
    parser.add_argument(
        "--ablate-image",
        action="store_true",
        help="Ablate the image to test model reliance on pointing coordinates.",
    )
    parser.add_argument(
        "--ablate-coords",
        action="store_true",
        help="Ablate one coordinate at a time in the text to test model reliance on them.",
    )

    parser.set_defaults(func=main)
    # set it automatically
    parser.set_defaults(open_ended_questions=True)
    parser.set_defaults(add_pointing_coords=True)


def main(args):
    if args.ablate_image:
        print("Ablating the image to test model reliance on pointing coordinates.")
    if args.ablate_coords:
        print("Ablating one coordinate at a time to test model reliance on them.")

    # get the model and processor based on the model name
    model_name, Model, Collator = MAP_MODELS_COORDS[args.model_name]
    ...
    test_ds = CIVETDataset(
        ...
        ids_to_process=(
            set(args.ids_to_process) if args.ids_to_process is not None else None
        ),
        add_coords=args.add_pointing_coords,
        ablate_image=args.ablate_image,
        ablate_coords=args.ablate_coords,
    )
    ...
    with torch.no_grad():
        for input_ids, input_texts, targets, sample_ids in tqdm(
            ...
            for sample_id, input, original_pred, target in zip(
                sample_ids, input_texts, preds, targets
            ):

                pred = match_number_in_text(original_pred)  # type: ignore
